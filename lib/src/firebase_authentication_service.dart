...
class FirebaseAuthenticationService {
  ...
  Future<FirebaseAuthenticationResult> signInWithGoogle() async {
    try {
      ...
      return FirebaseAuthenticationResult(user: result.user, isNewUser: result.additionalUserInfo?.isNewUser);
    } on FirebaseAuthException catch (e) {
      log?.e(e);
  ...
  Future<FirebaseAuthenticationResult> createAccountWithEmail({
    ...
    try {
      ...
      return FirebaseAuthenticationResult(user: result.user, isNewUser: true);
    } on FirebaseAuthException catch (e) {
      log?.e('A firebase exception has occured. $e');
      return FirebaseAuthenticationResult.error(
          exceptionCode: e.code.toLowerCase(),
...
class FirebaseAuthenticationResult {
  ...
  FirebaseAuthenticationResult({required this.user, required this.isNewUser})
      : errorMessage = null,
        exceptionCode = null;

  FirebaseAuthenticationResult.error({this.errorMessage, this.exceptionCode})
      : user = null, isNewUser = null;
